...
Read current `java-v4` activation status and pins only from `RELEASES.md` and the
trusted assessment workflow on the upstream protected default branch. The
immutable `baseline-v4` tag and candidate checkouts are snapshots: their
...
Start from the immutable `baseline-v4` snapshot and do not merge, rebase, or
...
`baseline-v4..candidate` and rejects every path other than the two above.
