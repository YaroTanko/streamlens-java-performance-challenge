...
and invariants. Live `java-v5` activation status and pins are authoritative only
in `RELEASES.md` and the trusted assessment workflow on the upstream protected
default branch. `baseline-v5` is the immutable candidate snapshot (**B**), so its
...
Exact `objects/op` is not a `java-v5` scored metric.
...
scope audit examines `baseline-v5..candidate` and permits only those two files.
...
before public `java-v5` activation; mocks are not release evidence. The manifest
