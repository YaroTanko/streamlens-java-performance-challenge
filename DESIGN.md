...
and invariants. Assessment version 2 (`java-v2`) is pending until the immutable
`baseline-v2`, workflow and image pins, exact-image real Docker-runtime canary,
calibration, and private-evaluator pre-score gate are complete.
...
Exact `objects/op` is not a `java-v2` scored metric.
...
build logic, scripts, and workflows therefore come from the baseline.
...
before `baseline-v2` activation; mocks are not release evidence. The manifest
