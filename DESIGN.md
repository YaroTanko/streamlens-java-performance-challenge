...
and invariants. Live `java-v6` activation status and pins are authoritative only
in `RELEASES.md` and the trusted assessment workflow on the upstream protected
default branch. `baseline-v6` is the immutable candidate snapshot (**B**), so its
...
Exact `objects/op` is not a `java-v6` scored metric.
...
scope audit examines `baseline-v6..candidate` and permits only those two files.
...
before public `java-v6` activation; mocks are not release evidence. The manifest
