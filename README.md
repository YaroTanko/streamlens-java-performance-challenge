...
> default branch. The immutable `baseline-v6` tag and every candidate branch are
...
>
> Before scoring, verify in that upstream record that the immutable `baseline-v6`
...
ended. Start from `baseline-v6` and do not merge, rebase, or otherwise update the
candidate branch from the upstream default branch; that branch later carries
mutable activation metadata. Scope validation compares `baseline-v6..candidate`
...
The protected upstream default branch becomes an active `java-v6` release only
