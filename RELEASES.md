...
## Current protected upstream default branch — `java-v6` pending activation

At this protected-default-branch revision, `java-v6` is not active and must not
be used to score candidates until the full release gate in `PRD.md` is met: an
immutable `baseline-v6` SHA, exact image digest, real Docker-runtime canary,
calibration evidence, and a reviewed private-evaluator pre-score gate.

### Immutable baseline snapshot

`baseline-v6` is the immutable candidate starting snapshot (**B**). It captures
...
file is read from the `baseline-v6` tag, its pending wording is historical and
