...
the immutable `baseline-v4` tag. Those snapshots intentionally retain their
...
- `baseline-v4` exists and its tag resolves to the workflow's full pinned SHA;
...
- retained calibration evidence and the required private-evaluator pre-score
  review are available for `java-v4`;
...
1. Record candidate, interviewer, assessment version, baseline SHA, start branch,
   ...
   the immutable `baseline-v4` snapshot (**B**), and that it has not merged,
