...
Do not run a scored interview while the repository says **pending activation**.
Before scheduling, verify from current repository/release evidence—not memory—that:

- `baseline-v2` exists and its tag resolves to the workflow's full pinned SHA;
...
- retained calibration evidence and the required private-evaluator pre-score
  review are available for `java-v2`;
...
1. Record candidate, interviewer, assessment version, baseline SHA, start branch,
   ...
   the immutable `baseline-v2` commit.
